...
sample_matrix <- data.frame(
                            matrix(
                                sample(pop, size = no_of_exp*no_of_iteration, replace = TRUE),
...
```{r}
library(ggplot2)
ggplot(sample_matrix, aes(x=means)) + geom_histogram(binwidth=0.3, fill="salmon", aes(y=..density..)) + stat_function(fun=dnorm, args=list(mean=mean(sample_matrix$means), sd=sd(sample_matrix$means)))
...
library(gridExtra)
plot1 <- ggplot(data = TG, aes(x = dose, y= len)) + geom_point(aes(color=supp))
plot2 <- ggplot(data = TG, aes(x = factor(dose), y= len)) + facet_grid(.~supp) + geom_boxplot(aes(fill=factor(dose)))
plot1
plot2
