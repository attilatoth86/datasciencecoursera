...
To verify the selected variables, I performed a different method for variable selection provided by `leaps` R library. This result proofed the previously selected three variables.

```{r echo=FALSE}
subset <- regsubsets(mpg ~ cyl+disp+hp+drat+wt+qsec+vs+am+gear+carb, mtc)
plot(subset, main = "Regression subset selection including exhaustive search")
```
