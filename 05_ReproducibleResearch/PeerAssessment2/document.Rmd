...
In my analysis I consider those events harmful that resulted any fatality or injury. Since 1950 in `r nrow(sdata_clean[sdata_clean$FATALITIES!=0,])` events were reported that resulted one or more people's death. In these cases a total of `r sum(sdata_clean$FATALITIES)` people deceased.

Injuries occured more often, reported `r nrow(sdata_clean[sdata_clean$INJURIES!=0,])` times in which `r sum(sdata_clean$INJURIES)` people were involved.
...
I sumed fatalities & injuries by event type and the most harmful proved to be the Tornado.
```{r}
...
In order to answer the question of which weather event has the greatest economic consequences, I needed to create the measurement of economic effect. In my analysis this measure is simply the sum amount of damages occured in properties & crop together.

Since 1950, `r nrow(sdata_clean[sdata_clean$PROPDMGUSD!=0 | sdata_clean$CROPDMGUSD!=0,])` weather events were registered that resulted any economic damage.
